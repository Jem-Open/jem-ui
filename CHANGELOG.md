...
- The `line` tab variant's active indicator now sits **on** the rail rather than floating above it, and is **`--pink-950`** rather than navy. The old approach used the trigger's own `border-b-2` pulled down by `-mb-px`, which cannot land on the rail while the list centres its items; it is a bar pinned 1px below the trigger instead. `--pink-950` and not the brand pink because a state indicator needs 3:1 (WCAG 1.4.11) and the brand pink is 2.77:1 on white against `--pink-950`'s 3.78:1 — the case those darker steps were added for.
